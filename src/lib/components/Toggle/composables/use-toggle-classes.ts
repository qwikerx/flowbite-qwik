...
import { InputSize } from '~/lib/components/Input/input-types'
...
export function useToggleClasses(size: Signal<InputSize>, color: Signal<string>) {
  ...
  const toggleColor = useComputed$(() => toggleColorClasses[color.value])
