...
export const AccordionHeader = component$<AccordionHeaderProps>(({ id, isVisible, flush, isLast, isFirst, onClick$, ...attrs }) => {
  const internalVisible = useComputed$(() => isVisible ?? false)
  const internalFlush = useComputed$(() => flush ?? false)
  const internalIsFirst = useComputed$(() => isFirst ?? false)
